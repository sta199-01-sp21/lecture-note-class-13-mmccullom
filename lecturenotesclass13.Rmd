...
We would use the statistic mean $\bar{x}$.
...
Point estimates only provide a single statistical value for a population that may be better represented by a range of values. They give less information when compared to intervals.
...
```{r data_manhattan}
manhattan <- read_csv("~/All Class Notes/lecture-note-class-13-mmccullom/manhattan.csv")
```
...
No because the mean value changes with every simulation.
...
- Sample: 1 Bedroom Apartments on Craigslist
- Sample size: 20
...
There is 95% confidence that the actual mean price of a Manhattan Apartment lies in this range of mean values.
...
- Population: Mice
- Parameter of interest: Proportion of dead mice with no treatment
- Sample: Dead mice with no treatment
- Sample size: 522
...
There is a confidence of 95% that the proportion of dead mice not on treatment is between 0.690 and 0.797
...
boot_live <- mice%>%
  filter(treat==1)%>%
  mutate(living = if_else(died==0, "yes", "no"))%>%
  specify(response=living, success = "yes")%>%
  generate(reps=10000, type = "bootstrap")%>%
  calculate(stat = "prop")
```

```{r practice_2_ci}
ci_boot_live <- boot_live %>%
  summarize(
...
visualize(boot_live)+
  shade_ci(ci_boot_live)
```


Give an interpretation of your result.

We have a 90% confidence level that the actual mean is between 0.551 and 0.651, meaning that between 0.551 and 0.651 mice who received the treatment lived.
